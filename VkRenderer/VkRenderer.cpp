...
{
// Contains info in regards to the Command Pool
...
VkCommandBufferAllocateInfo cmdBufAllocateInfo = 
vkTools::initializers::commandBufferAllocateInfo(_cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, (uint32_t)_drawCmdBuffers.size());
...
{
// Bitmask indicating which aspect(s) of the image are included in the view
...
for (uint32_t i = 0; i < _drawCmdBuffers.size(); i++) {
...
presentToClearBarrier.image = _drawCmdBuffers[i].image;
...
clearToPresentBarrier.image = _drawCmdBuffers[i].image;
...
postPresentBarrier.subresourceRange = subResourceRange;
postPresentBarrier.image = _swapChain.buffers[i].image;
...
// The following code is temporary code
