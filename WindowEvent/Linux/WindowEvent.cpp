...
WindowEvent::WindowEvent(const Obake::EventList& evRef_) : ASystem(evRef_), _connection(nullptr), _screen(nullptr) {
}
...
void WindowEvent::refresh() {
	...
	if (_lastEvent->response_type == XCB_EXPOSE) {
		...
		destroyWindow();	
	} else if (_lastEvent->response_type == XCB_KEY_RELEASE) { // "press" seems ignored while "released" seems to be called while pressed...
...
int main() {
	Obake::Core c;
	sleep(1);
	System::WindowEvent winEvent(c.getEventListRef());

	winEvent.createWindow();
	// Run the message loop.
	while (winEvent.alive()) {
		winEvent.refresh();
	}
	return 0;
}
